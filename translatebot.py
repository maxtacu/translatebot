...
def detect_ru(text):
	languages = [Language.ENGLISH, Language.RUSSIAN, Language.PORTUGUESE]
...
def start(message):
	...
	bot.send_message(message.chat.id, f"Hello {message.from_user.first_name}, welcome to Deepl Translate bot!")
		...
		if detect_ru(remove_emoji(message.text)):
			...
			if len(joint_translated_text) < TEXT_LIMIT:
				bot.edit_message_text(
					message_id=message.message_id, chat_id=message.chat.id, text=joint_translated_text
				)
			else:
		...
		if detect_ru(message.caption):
			...
			if len(joint_translated_text) < CAPTION_LIMIT:
				bot.edit_message_caption(
					message_id=message.message_id, chat_id=message.chat.id, caption=joint_translated_text
				)
			else:
